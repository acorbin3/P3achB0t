...
enum class Quest {
    ...
    private val varbit: Varbits?
    private val varPlayer: VarPlayer?

    constructor(id: Int, questName: String, varbit: Varbits?) {
        this.id = id
    ...
    constructor(id: Int, questName: String, varPlayer: VarPlayer?) {
        this.id = id
        this.questName = questName
        varbit = null
    ...
    fun getVar(ctx: Context): Int {
        return if (varbit != null) {
            ctx.vars.getVarbit(varbit.id)
        } else {
            varPlayer?.id?.let { ctx.vars.getVarp(it) } ?: 0
        }
    }
}