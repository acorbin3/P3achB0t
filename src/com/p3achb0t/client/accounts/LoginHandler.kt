...
class LoginHandler(var account: Account = Account()) {
    ...
    suspend fun login(ctx: Context): Boolean {
        if(!ctx.widgets.isWidgetAvaliable(378, 87) && !ctx.worldHop.isLoggedIn) {
            ...
            ctx.mouse.moveMouse(Point(ctx.client.get__dv_z(), 310), true, Mouse.ClickType.Left)
            delay(500)
            ctx.mouse.moveMouse(Point(ctx.client.get__dv_z() + 55, 294), true, Mouse.ClickType.Left)
            ...
            if (ctx.client.getLogin_username() != account.username) {
              ...
              ctx.mouse.moveMouse(Point(Random.nextInt(ctx.client.get__dv_z() - 50, ctx.client.get__dv_z() + 70), Random.nextInt(240, 249)), true, Mouse.ClickType.Left)
            ...
            timeout.reset()
            //Move to password
            ctx.mouse.moveMouse(Point(Random.nextInt(ctx.client.get__dv_z() - 50, ctx.client.get__dv_z() + 70), Random.nextInt(258, 269)), true, Mouse.ClickType.Left)
            ...
            if ((LoginResponse.getLoginResponse(ctx) == LoginResponse.BANNED) ||
                ...
                if(LoginResponse.getLoginResponse(ctx) == LoginResponse.INVALID){
                    ...
                    ctx.mouse.moveMouse(Point(Random.nextInt(ctx.client.get__dv_z() - 12, ctx.client.get__dv_z() - 6), Random.nextInt(260, 290)), true, Mouse.ClickType.Left)
                }
